...
void Text::FormatNone()
{
	...
	for( const auto c : m_baseString )
	{
		...
		{
			str.insert( size_t( curCharPos ), m_layout.tabSpaceCount, ' ' );
			curCharPos += int32_t( m_layout.tabSpaceCount );
		}
	}
...
void Text::UpdatePositionsY( size_t yStart, size_t NumLines, size_t CharHeight )
{
	...
	{
		m_positions[ i ].y = int32_t( yStart + ( i * CharHeight ) );
	}
}
