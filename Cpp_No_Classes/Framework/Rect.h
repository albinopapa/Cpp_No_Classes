...
constexpr RectType Rectify( const RectType& Source, const RectType& Bounds, bool Translate = false )
{
	if( Translate )
	{
		return RectType(
			std::max( -Source.left, Bounds.left ),
			std::max( -Source.top, Bounds.top ),
			std::min( Bounds.right - Source.left, Source.GetWidth() ),
			std::min( Bounds.bottom - Source.top, Source.GetHeight() ) 
		).Translate( Source.LeftTop() );
	}
